...
def backup_db(user, password, hostname, db_name, output_file, compression, logger=None):
    ...
    alembic_version = (
        client.session.query(client.models.alembic_version).one().version_num
    )

    output_file.write(f"-- alembic_version={alembic_version}\n".encode())
    output_file.write(
        (
            f"-- backup of {hostname}/{db_name} by "
    ...
    command = (
        f"pg_dump --data-only -Z {compression} --no-owner "
...
def restore_db(user, password, hostname, db_name, alembic_conf, dump_file):
    ...
    conf = get_alembic_conf(
        f"postgresql://{user}:{password}@{hostname}/{db_name}", alembic_conf
    )
    alembic_upgrade(conf, alembic_version)

    os.environ["PGPASSWORD"] = password
    command = (
        f"pg_restore -x --data-only --if-exists --no-owner -t "
